...
var insertSocialLinkSchema = createInsertSchema(socialLinks).omit({ id: true });
var insertClickEventSchema = createInsertSchema(clickEvents).omit({ id: true });

// server/routes.ts
async function registerRoutes(app2) {
  app2.get("/api/social-links", async (_req, res) => {
    const links = await storage.getSocialLinks();
    res.json(links);
  });
  app2.post("/api/click-event", async (req, res) => {
